...
GWS.gws4k.cmd.agc = ''
GWS.gws4k.cmd.note = 'gettemp | awk \'{print $2}\' | tr -d "\n"'

function GWS.gws4k.update()
	local _gws = {}
	
	_gws.rgn = cmd.exec(GWS.gws4k.cmd.region) or 0
	_gws.ch = cmd.exec(GWS.gws4k.cmd.channel) or 0
	_gws.txpwr = cmd.exec(GWS.gws4k.cmd.txpwr) or 0
	_gws.rxg = -1
	_gws.tpc = -1
	_gws.agc = -1
...
GWS.gws5k = {}
GWS.gws5k.cmd = {}
GWS.gws5k.cmd.rfinfo = 'rfinfo | grep [0-9\.\-] -o'
...
function GWS.ops.Update()
	local _result
	local _fmt = '{"rgn": %d, "ch": %d, "rxg": %d, "txpwr": %d, "tpc": %d, "agc": %d, "note": "%s" }'

	local _gws = GWS.ops.read()
	_result = string.format(_fmt, _gws.rgn or 0, _gws.ch or 0, _gws.rxg or 0, _gws.txpwr or 0, 
		_gws.tpc or -1, _gws.agc or -1, _gws.note or '')
